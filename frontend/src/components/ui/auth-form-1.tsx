...
export function Auth({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      ...
      // Here we'll just mock it or decode if it's an ID token
      // Since it's an access token from useGoogleLogin, we might need to fetch user info
...
function AuthSocialButtons({ isLoading, onMicrosoftLogin, onGoogleLogin }: AuthSocialButtonsProps) {
  return (
    <div
...
function AuthSignIn({ onForgotPassword, onSignUp, onMicrosoftLogin, onGoogleLogin }: AuthSignInProps) {
  const { login } = useAuth();
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Button
              ...
              variant="ghost"
              size="icon"
              className="absolute right-1 top-0 h-full text-muted-foreground/60 hover:text-white hover:bg-transparent"
              onClick={() =>
...
function AuthSignUp({ onSignIn, onMicrosoftLogin, onGoogleLogin }: AuthSignUpProps) {
  const { login } = useAuth();
  const [formState, setFormState] = React.useState<FormState>({
    isLoading: false,
    error: null,
  ...
  const onSubmit = async (data: SignUpFormValues) => {
    ...
    try {
      ...
      login({
        ...
        name: data.name,
        image: `https://api.dicebear.com/7.x/avataaars/svg?seed=${data.email}`,
      }, true); // isNewUser = true for Sign Up
    } catch {
      setFormState((prev) => ({ ...prev, error: "An unexpected error occurred" }));
    } finally {
      setFormState((prev) => ({ ...prev, isLoading: false }));
