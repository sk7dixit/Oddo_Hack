  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-4xl mx-auto">
      <div className="space-y-16">
        ...
        {/* Editor Section */}
        <section className="bg-white rounded-[3rem] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.03)] overflow-hidden">
          <div className="px-10 py-8 bg-slate-900 text-white flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-blue-600 rounded-2xl flex items-center justify-center text-xl shadow-lg shadow-blue-900/20">✍️</div>
              <h2 className="text-xl font-black tracking-tight">Trip Journal</h2>
            </div>
            <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">Entry No. {notes.length + 1}</span>
          </div>
          ...
          <div className="px-12 py-8 bg-slate-50 flex items-center justify-between border-t border-slate-100">
            <div className="flex flex-col">
              <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">Status</span>
              <span className="text-xs font-black text-blue-600 uppercase tracking-widest leading-none">Ready to Sync</span>
            </div>
            <button 
              onClick={handleAddNote} 
              disabled={!content.trim() || isSaving}
              className="bg-blue-600 hover:bg-blue-700 text-white font-black px-12 py-4 rounded-2xl transition-all shadow-xl shadow-blue-100 disabled:opacity-50"
            >
        ...
        <section className="space-y-12">
          ...
          {notes.length === 0 ? (
            <div className="py-20 text-center grayscale opacity-30">
               ...
               <p className="text-sm font-black uppercase tracking-widest">Journal Empty</p>
            </div>
          ) : (
            <div className="space-y-10">
              {notes.map((note) => (
                <div 
                  key={note._id} 
                  className={`relative bg-white p-12 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-slate-200/40 transition-all duration-500 hover:-translate-y-2 ${note.isOptimistic ? "opacity-50 grayscale" : ""}`}
                >
                  <div className="flex items-start gap-10">
                    <div className="flex-shrink-0 text-center">
                       <div className="w-20 h-20 rounded-3xl bg-slate-50 border border-slate-100 flex flex-col items-center justify-center">
                          <span className="text-[10px] font-black text-slate-300 uppercase mb-1">{new Date(note.createdAt).toLocaleDateString('en-US', { month: 'short' })}</span>
                          <span className="text-3xl font-black text-slate-800 leading-none">{new Date(note.createdAt).getDate()}</span>
                       </div>
                    </div>
                    <div className="flex-1 space-y-6 pt-2">
                      <p className="text-4xl font-black text-slate-800 leading-[1.1] tracking-tighter">
                        {note.content}
                      </p>
